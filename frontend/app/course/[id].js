...
import { courseAPI, enrollmentAPI, lessonAPI } from '../../src/services/api';
...
export default function CourseDetailScreen() {
  ...
  const fetchCourseData = async () => {
    try {
      const [courseRes, lessonsRes] = await Promise.all([courseAPI.getOne(id), lessonAPI.getByCourse(id)]);
    ...
    } catch (error) { Alert.alert('Error', 'Failed to load course'); } finally { setLoading(false); }
  };
  ...
  const handleEnroll = async () => {
    ...
    } catch (error) { Alert.alert('Error', error.response?.data?.message || 'Failed to enroll'); } finally { setEnrolling(false); }
  };
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.header}>
        <View style={styles.thumbnail}><Ionicons name="play-circle" size={60} color={COLORS.white} /></View>
        <View style={styles.headerInfo}>
          <Text style={styles.title}>{course.title}</Text>
          <Text style={styles.instructor}>By {course.instructor?.name}</Text>
          <View style={styles.metaRow}>
            <View style={styles.badge}><Ionicons name="star" size={14} color={COLORS.secondary} /><Text style={styles.badgeText}>{course.rating || 'New'}</Text></View>
            <View style={styles.badge}><Ionicons name="people" size={14} color={COLORS.gray[500]} /><Text style={styles.badgeText}>{course.enrolledCount} students</Text></View>
            <View style={styles.badge}><Ionicons name="time" size={14} color={COLORS.gray[500]} /><Text style={styles.badgeText}>{Math.round(course.duration / 60)}h</Text></View>
          </View>
      ...
      <View style={styles.section}>
        ...
        {lessons.map((lesson, index) => (
          <TouchableOpacity key={lesson._id} style={styles.lessonItem}
            onPress={() => enrollment ? router.push({ pathname: `/lesson/${lesson._id}`, params: { enrollmentId: enrollment._id } }) : null}>
      ...
      <View style={styles.bottomBar}>
        {enrollment ? (
          <View style={styles.progressContainer}>
            <View style={styles.progressBar}><View style={[styles.progressFill, { width: `${enrollment.progress}%` }]} /></View>
