...
import { lessonAPI, enrollmentAPI } from '../../src/services/api';
...
export default function LessonScreen() {
  const { id, enrollmentId } = useLocalSearchParams();
  ...
  const fetchLesson = async () => {
    try { const res = await lessonAPI.getOne(id); setLesson(res.data); }
    catch (error) { Alert.alert('Error', 'Failed to load lesson'); } finally { setLoading(false); }
  };

  const handleComplete = async () => {
    try {
      await enrollmentAPI.updateProgress(enrollmentId, id);
      Alert.alert('Success', 'Lesson marked as complete!', [{ text: 'OK', onPress: () => router.back() }]);
  ...
  if (!lesson) return <View style={styles.center}><Text>Lesson not found</Text></View>;
    ...
    <ScrollView style={styles.container}>
      {youtubeId ? (
        <View style={styles.videoContainer}>
          <YoutubePlayer height={220} play={playing} videoId={youtubeId} onChangeState={onStateChange} />
        </View>
      ) : lesson.videoUrl ? (
        <TouchableOpacity style={styles.videoPlayer} onPress={() => Linking.openURL(lesson.videoUrl)}>
