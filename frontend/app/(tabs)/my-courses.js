...
export default function MyCoursesScreen() {
  ...
  useFocusEffect(
    useCallback(() => { fetchEnrollments(); }, [])
  );

  const fetchEnrollments = async () => {
    try { const res = await enrollmentAPI.getMyEnrollments(); setEnrollments(res.data); }
  ...
  const renderEnrollment = ({ item }) => (
    <TouchableOpacity style={styles.card} onPress={() => router.push(`/course/${item.course?._id}`)}>
  ...
  return (
    <View style={styles.container}>
      <FlatList data={enrollments} renderItem={renderEnrollment} keyExtractor={(item) => item._id}
